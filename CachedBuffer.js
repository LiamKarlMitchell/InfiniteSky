      ...
      do {
        ...
        if (this.bufferLength - position >= this.HeaderSize) // If has enough space for header and packet id
        {
          ...
          if (packet != null) {
            ...
            console.log('PacketID: '+packetID+' '+packetID.toString(16).toUpperCase()+' bufsize: '+(this.bufferLength-position)+' '+packet.function.name);
          }
            ...
            if (!packet.Restruct) { // Check if not a restruct packet
              ...
              try {
                packet.function(this, packet.Restruct.unpack(this.data.slice(position, position + packet.Restruct.size)), packetID);
              }
              catch (ex) {
                // For testing only...
                this.bufferLength = 0;
