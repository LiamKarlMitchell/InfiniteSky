...
GMCommands.AddCommand(new Command('sendmail',0,function command_sendmail(string,client){	
	var fromEmail = client.account.Email || util.config.smtpTransport.auth.user;
	var fromUser = client.account.Username + '/' + client.character.Name;

	var mail = {
	    from: fromUser+" <"+util.config.smtpTransport.auth.user+">", // sender address
	    to: util.config.smtpTransport.auth.user, // list of receivers
	    'reply-to': fromEmail,
