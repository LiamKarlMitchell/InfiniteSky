...
socket.Teleport = function(Location, ZoneID) {
    ...
    this.character.state))), util.config.viewable_action_distance);
    return true;
}
...
world.loadAllZones = function() {
    ...
    if (util.config.Zones !== undefined) {
        var mapLoadQueue = async.queue(world.loadZone, util.config.AsyncZoneLoadLimit || 4);

        for (var id in util.config.Zones) {
            if (util.config.Zones.hasOwnProperty(id) && !isNaN(id)) {
...
world.loadZone = function(ZoneID,callback) {
    ...
    if (zones[ZoneID] && util.config.Zones[ZoneID].Load == false) {
        // TODO: Unload the zone.
        callback(null,ZoneID)
        return;
    }

    if (zones[ZoneID] === undefined && util.config.Zones[ZoneID].Load) {
    ...
    if (AreAllInfosLoaded()) {
        ...
        var infoWaitEvent = function (what) {
            switch (what) {
                ...
                case 'Zone': loadedInfos['Zone'] = typeof(Zone) !== undefined; break;
            }
